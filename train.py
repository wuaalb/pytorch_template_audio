...
def train(args):
    ...
    # dataset
    file_list_trn = load_file_list(dsetdir / 'filelist_trn.txt', multicol=False)
    ...
    file_list_val = load_file_list(dsetdir / 'filelist_val.txt', multicol=False)
    ...
    melspec_fn = MelSpec(hparams.sr, hparams.hop_len, hparams.win_len, hparams.fft_len, hparams.window_kind,
                         hparams.n_mel, hparams.fmin, hparams.fmax, norm_window=True, filter_norm=1, center=True, pad_mode='reflect', clamp_min_db=hparams.clamp_min_db, to_db=False)
